...
abstract class DifferentTouchInput
{
	...
	private static class MultiTouchInput extends DifferentTouchInput
	{
		...
		public void process(final MotionEvent event)
		{
			...
			{
				// it is possible that multiple pointers return that event, but we're handling only pointer #0
		...
		public void processGenericEvent(final MotionEvent event)
		{
			...
			*/
			...
			if( (event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP ||
				(event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_CANCEL )
				down = 0;
			// we're processing only one touch pointer, touchpad will most probably support multitouch
...
class DemoRenderer extends GLSurfaceView_SDL.Renderer
{
	...
	public void DrawLogo(GL10 gl)
	{
		...
		{
			bmp = new BitmapDrawable(context.getResources().openRawResource(R.drawable.picture_frame));
		}
