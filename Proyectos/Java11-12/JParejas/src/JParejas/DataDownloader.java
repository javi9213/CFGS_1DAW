	...
	public boolean DownloadDataFile(final String DataDownloadUrl, final String DownloadFlagFileName, int downloadCount, int downloadTotal)
	{
		...
		if( checkFile != null )
		{
			try {
				...
				Status.setText( res.getString(R.string.dialog_alert_title) );
				return true;
			} catch ( IOException e ) {};
		}
		checkFile = null;
		...
		while( downloadUrlIndex < downloadUrls.length )
		{
			...
			Status.setText( downloadCount + "/" + downloadTotal + ": " + res.getString(R.string.ok, url) );
		...
		if( FileInAssets )
		{
			...
			else
			{
				try {
					...
					Status.setText( res.getString(R.string.httpErrorUnsupportedScheme, url) );
					return false;
				}
			}
		}
		else
		{
			if( response == null )
			{
				...
				Status.setText( res.getString(R.string.httpErrorBadUrl, url) );
				return false;
			}

			Status.setText( downloadCount + "/" + downloadTotal + ": " + res.getString(R.string.emptyPhoneNumber, url) );
		...
		if(DoNotUnzip)
		{
			...
			if( out == null )
			{
				Status.setText( res.getString(R.string.VideoView_error_text_unknown, path) );
			...
			try {
				...
				while (len >= 0)
				{
					...
					Status.setText( downloadCount + "/" + downloadTotal + ": " + res.getString(R.string.status_bar_notification_info_overflow, percent, path) );
				}
				out.flush();
				out.close();
				out = null;
		...
		Status.setText( downloadCount + "/" + downloadTotal + ": " + res.getString(R.string.yes) );
